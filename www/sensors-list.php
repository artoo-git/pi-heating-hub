...
///////////////////////////////////////////////////////////
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
	    ...
	    $subnet_inet = shell_exec('ifconfig wlan0 |grep "inet "| awk "{print $2}" |cut -d/ -f1');

            $subnet_mask = shell_exec('ifconfig wlan0 |grep "inet "| awk "{print $4}" |cut -d/ -f1');
	    ...
	    $subnet_scan = shell_exec("nmap -sP $subnet_cidr | grep report|grep -v router| awk '{print $5}'|cut -d/ -f1");
            echo $subnet_scan;
	    ...
	    foreach( $subnet_devices as $device_ip ) {
                ...
                $sensor_count = file_get_contents("http://".$device_ip."/count.php");
		...
		if( $sensor_count > 0 ) {
                    ...
                    for ($sensor_ref =1 ; $sensor_ref <= $sensor_count; $sensor_ref++) { 
                        ...
                        $sensor_name = file_get_contents("http://".$device_ip."/name.php?id=".$sensor_ref);
                        $sensor_unit = "deg C";
